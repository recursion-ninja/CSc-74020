...
Furthermore, the model requires careful transformation of inputs to be either binary indicators or real values within the the range \NumericRange{0}{1}.
Not enough time was given to this model, ensuring inputs were appropriately tansformed and that the model selection was appropriate.
...
Gradient boosting model performed well for \TERC but did not scale well to \TER.
...
We believe that more exploration with each of these \Qty{6} models are warranted.
...
Overall, the predictive capabilities for smallest tier list \TERC is somewhat significant, though far from optimal.
With roughly \Qty{0.48} ``correctness'' for a given metric, the top classifiers out perform any uniformed guessing ($\Qty{0.08\overline{3}} = \frac{\Qty{1}}{\Qty{12}}$).
